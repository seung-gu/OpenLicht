...
// the memory buffer for the LCD. Displays Adafruit logo by pixel
...
void ssd1306_i2c::ssd1306_setBuffer(int x, int y, unsigned int value)
{
	buffer[x + SSD1306_LCDWIDTH*y] = value;
}


// the most basic function, set a single pixel
void ssd1306_i2c::ssd1306_drawPixel(int x, int y, unsigned int color)
{
	...
	switch (color) {
	case WHITE:
		buffer[x + (y / 8) * SSD1306_LCDWIDTH] |= (1 << (y & 7));	//led white
		break;
	case BLACK:
		cout<<"BLACK"<<endl;
		buffer[x + (y / 8) * SSD1306_LCDWIDTH] &= ~(1 << (y & 7));	//led black
		break;
	case INVERSE:
		cout<<"INVERSE"<<endl;
...
// Init SSD1306
void ssd1306_i2c::ssd1306_begin(unsigned int vccstate, unsigned int i2caddr)
{
	// I2C Init	
...
void ssd1306_i2c::ssd1306_invertDisplay(unsigned int i)
{
	if (i) {
...
void ssd1306_i2c::ssd1306_command(unsigned int c)
{
	// I2C
...
void ssd1306_i2c::ssd1306_display(int end_y, int end_x)
{
...
*/	
	for(int rows = 0; rows < end_y; rows++)
	{
		for(int cols = 0; cols < end_x; cols++)
		{
			wiringPiI2CWriteReg8(i2cd, 0x40, buffer[rows*SSD1306_LCDWIDTH + cols]); 	
		}
	}
...
void ssd1306_i2c::ssd1306_startscrollright(unsigned int start, unsigned int stop)
{
...
// Hint, the display is 16 rows tall. To scroll the whole display, run:
// ssd1306_scrollright(0x00, 0x0F)
void ssd1306_i2c::ssd1306_startscrollleft(unsigned int start, unsigned int stop)
{
...
// Hint, the display is 16 rows tall. To scroll the whole display, run:
// ssd1306_scrollright(0x00, 0x0F)
void ssd1306_i2c::ssd1306_startscrolldiagright(unsigned int start, unsigned int stop)
{
...
// Hint, the display is 16 rows tall. To scroll the whole display, run:
// ssd1306_scrollright(0x00, 0x0F)
void ssd1306_i2c::ssd1306_startscrolldiagleft(unsigned int start, unsigned int stop)
{
	ssd1306_command(SSD1306_SET_VERTICAL_SCROLL_AREA);
	ssd1306_command(0X00);
	ssd1306_command(SSD1306_LCDHEIGHT);
...
void ssd1306_i2c::ssd1306_stopscroll(void)
{
	ssd1306_command(SSD1306_DEACTIVATE_SCROLL);
}
...
void ssd1306_i2c::ssd1306_dim(unsigned int dim)
{
...
// clear everything
void ssd1306_i2c::ssd1306_clearDisplay(void)
{
	memset(buffer, 0,
...
void ssd1306_i2c::ssd1306_drawFastHLine(int x, int y, int w, unsigned int color)
{
...
void ssd1306_i2c::ssd1306_drawFastVLine(int x, int y, int h, unsigned int color)
{
	unsigned int bSwap = false;
	switch (rotation) {
	case 0:
...
void ssd1306_i2c::ssd1306_fillRect(int x, int y, int w, int h, int fillcolor)
{
...
void ssd1306_i2c::ssd1306_setTextSize(int s)
{
	textsize = (s > 0) ? s : 1;
}
...
void ssd1306_write(int c)
{
	if (c == '\n') {
		...
		ssd1306_i2c::ssd1306_drawChar(cursor_x, cursor_y, c, WHITE, textsize);
		cursor_x += textsize * 6;
...
void ssd1306_i2c::ssd1306_drawString(string str)
{
	for (int i = 0; i < str.size(); i++)
		ssd1306_write(str[i]);
}
/*
void ssd1306_i2c::ssd1306_drawString(char *str)
{
	int i, end;
	end = strlen(str);
	for (i = 0; i < end; i++)
...
*/
// Draw a character
void ssd1306_i2c::ssd1306_drawChar(int x, int y, unsigned char c, int color, int size)
{
